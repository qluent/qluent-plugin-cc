---
name: qluent-tree-protocol
description: Canonical protocol for the metric-tree workflow — tree resolution, window reuse, AnalysisRun handles, Shapley and confidence, elasticity guardrails, the unsupported-cut fallback, and report precedence. Loaded by /qluent:investigate, /qluent:deep-dive, /qluent:visualize and the tree agents; user-invocable for protocol inspection.
user-invocable: true
---

# Qluent metric-tree protocol

This is the deep half of the qluent protocol: everything that only applies once
a question has been routed to a metric tree. The routing decision itself, the
provenance labels, the grounding rules and the session workspace live in the
`qluent-interpretation` skill, which every qluent workflow loads. Load this
skill on top of it when the work touches a tree.

The query workflow never needs anything below. `/qluent:query` deliberately
does not load this file.

This skill is the single source of truth for tree protocol. Commands and
...
The grounding rules in the `qluent-interpretation` skill apply here too. On a
tree they also fix the order the evidence has to arrive in:
...
Decompositions, segment rankings, and elasticity estimates are deterministic
fields, never something to reconstruct from a narrower result.
...
## Session paths — the tree workspace files

The workspace itself (`$QLUENT_DIR`, the prelude that resolves it, and the
rule against machine-global rendezvous paths) is declared in the
`qluent-interpretation` skill, alongside the files the query path uses. The
three files below belong to the tree workflow and are declared here, so the
query path does not carry them. Each file is declared exactly once; the set of
files allowed to mention each one is pinned by `tests/test_session_paths.sh`,
and adding a new consumer requires updating that allowlist on purpose.
