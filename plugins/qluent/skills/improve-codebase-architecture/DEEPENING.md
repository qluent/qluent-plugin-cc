...
The *one adapter / two adapters* rule from [LANGUAGE.md](LANGUAGE.md) governs
when to introduce a port: only when at least two adapters are justified
...
by its own tests) as well as the **external seam** at its interface. Don't
